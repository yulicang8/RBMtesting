...
Tsum.prototype.useRapunzelPlusSkill = function () {
    ...
    // Row 1
    path.push({x: 15,  y: 95});
    path.push({x: 185, y: 95});

    // Down
    path.push({x: 185, y: 120});

    // Row 2
    path.push({x: 15,  y: 120});

    // Down
    path.push({x: 15,  y: 145});

    // Row 3
    path.push({x: 185, y: 145});

    debug("Rapunzel+");

    this.linkTsums(path);

    // Wait for bubble generation
    this.sleep(2800);

    // Reuse Robotmon's bubble clearing
